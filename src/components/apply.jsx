    ...
    getColumns = () => {
        ...
        columns[columns.length-3].render = (text,record,index) => {
            switch(record.confirm) {
                case 0 : return <span>{"处理中"}</span>; break;
                case 1 : return <span>{record.status===-1?"撤销":record.status===0?"全部成功":record.status===1?"部分处理":record.status===2?"待处理":record.status===3?"处理中":record.status===4?"拒绝处理":record.status===5?"待提交":record.status===6&&"代发失败"}</span>
            }
        }
